...
try:
    ...
    for x in range(1,5):
        ...
        if objTab.Session.Connected == True:
            ...
            objTab.Screen.Send("show macfilter summary" + chr(13))
            stats = objTab.Screen.ReadString(")")
            if 'Entries............................... 0' in stats:
                objTab.Session.Disconnect()
                f = open(writefile,"w+")
                f.write("There are no MAC addresses in the system.")
                f.close()
            else:
                if '(Javits-' in stats:
                    n = False
                ...
                while n:
                    ...
                    stats = objTab.Screen.ReadString("y")
                    f = open(writefile,"a+")
                    f.write(str(stats))
                    f.close()
                    if ">" not in str(stats):
